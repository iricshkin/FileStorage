...
from cache.redis import connection
...
@router.get('/ping')
async def ping(*, db: AsyncSession = Depends(get_session)):  # type: ignore
    try:
        ...
        redis_ping = 'No access'  # type: ignore
    try:
        start = time.time()
        ...
        if conn:
            ...
            return {'DB connection': f'{db_ping}', 'Redis': f'{redis_ping}'}
    except Exception:
        db_ping = 'No access'  # type: ignore
