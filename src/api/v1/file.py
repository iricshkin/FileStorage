...
from src.cache.redis_utils import get_cache, get_cache_or_data, redis_cache, set_cache
from src.core.config import app_settings
from src.db.db import get_session
from src.schemas.file import FilesList, FileStorage, FileStorageInDB
...
from src.utils.filestorage import get_compressed_file_with_media_type, get_file_info, is_downloadable
...
async def get_list(
    *,
    ...
    if not data:
        ...
        files_lst = [FileStorage.from_orm(file).dict() for file in files]
        data = {'account_id': current_user.id, 'files': files_lst}
...
@router.post(
    '/upload',
    response_model=FileStorageInDB,
...
async def upload_file(
    *,
    path: str = Query(
        description='Enter path to directory OR file, ' 'both start with /'
    ),
    db: AsyncSession = Depends(get_session),
    current_user: CurrentUser = Depends(get_current_user),
    ...
    file_obj = await file_crud.create_or_put_file(
        db=db, user_obj=current_user, file_obj=file, file_path=full_path
    )
...
async def download_file_by_path_or_id(
    *,
    ...
    current_user: CurrentUser = Depends(get_current_user),
    path: str = Query(
        description='Query of file path (starts with /) OR file id, '
        'for compression available directory path OR '
        'directory id'
    ),
    compression_type: Optional[str] = Query(
        default=None,
    ...
    if not compression_type:
        ...
        file_info = await get_cache_or_data(
            ...
            data_schema=FileStorage,
            db_func_args=(db, path),
        )
        is_downloadable(file_info=file_info)
        file_url = app_settings.files_folder_path + file_info.get('path')
    ...
    refreshed_io, media_type = await get_compressed_file_with_media_type(
        db=db, cache=cache, path=path, compression_type=compression_type
    )
